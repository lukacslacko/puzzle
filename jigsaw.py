...
def linear_tab(start: complex, end: complex, radius: float, path: Path, *, left: bool) -> complex:
...
def circular_tab_inwards(center: complex, start_direction: complex, end_direction: complex, towards: complex, corner_radius: float, radius: float, path: Path) -> complex:
    ...
    dot_product = towards_vector.real * middle_vector.real + towards_vector.imag * middle_vector.imag
    if dot_product < 0:
        middle_vector *= -1
    rounding_radius = radius * 0.2
    a = corner_radius - rounding_radius
    ...
    path.arc(rounding_radius, along(start_rounding_center, tab_center, rounding_radius))
    path.arc(radius, along(end_rounding_center, tab_center, rounding_radius), large_arc=True, sweep=False)
    path.arc(rounding_radius, end_touch_point)
