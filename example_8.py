...
with svg.SVG(0, 3, 700, 300, "a.svg") as s:
    with s.transformation(shift=-4.5, rotate=0):
        path = svg.Path(AA)
        path.line(A)
        path.circular_tab(0, B, B, 0.15, inwards=True)
        path.line(BB)
        s.draw_path(path, svg.COLORS[0])

        path = svg.Path(BB)
        path.line(B)
        path.circular_tab(0, C, C, 0.15, inwards=True)
        path.line(CC)
        s.draw_path(path, svg.COLORS[1])

        path = svg.Path(CC)
        path.line(C)
        path.circular_tab(0, D, D, 0.15, inwards=True)
        path.line(DD)
        s.draw_path(path, svg.COLORS[2])

        path = svg.Path(DD)
        path.line(D)
        path.circular_tab(0, A, A, 0.15, inwards=True)
        path.line(AA)
